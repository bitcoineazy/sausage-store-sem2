...
|-|-|-|
| `ng build --prod` падает: `Property 'collapsed' is private` | AOT-компиляция не видит приватное поле из шаблона | поле сделано публичным |
| бэкенд не стартует: `Cannot create authentication mechanism for TOKEN` | Spring Cloud Vault включён по умолчанию и требует токен | токен и адрес Vault заданы в properties/окружении |
| Job `mongodb-init` падает без логов | `OOMKilled`: mongosh не укладывается в 128 Mi | лимит поднят до 300 Mi |
| `helm upgrade` → `exceeded quota: services=5` | Service у `backend-report` не нужен, а квота на Service исчерпана | Service удалён |
| `helm upgrade` → `exceeded quota: secrets=10` | каждая ревизия релиза хранится как Secret | `--history-max 2` |
